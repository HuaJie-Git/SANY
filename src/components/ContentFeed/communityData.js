...
export const users = new Proxy([], {
  get(target, prop, receiver) {
    const current = isGuestModeActive() ? GUEST_USERS : BASE_USERS;
    if (prop === Symbol.iterator) return current[Symbol.iterator].bind(current);
    const val = Reflect.get(current, prop, receiver);
    return typeof val === 'function' ? val.bind(current) : current[prop];
  },
});

export const getUserById = (id) => {
  const list = isGuestModeActive() ? GUEST_USERS : BASE_USERS;
  return list.find((u) => u.id === id) || GUEST_USERS.find((u) => u.id === id) || null;
};
...
export const topics = new Proxy([], {
  get(target, prop, receiver) {
    const current = isGuestModeActive() ? GUEST_TOPICS : BASE_TOPICS;
    if (prop === Symbol.iterator) return current[Symbol.iterator].bind(current);
    const val = Reflect.get(current, prop, receiver);
    return typeof val === 'function' ? val.bind(current) : current[prop];
  },
...
export const getTopicByName = (name, demoMode = isGuestModeActive()) => {
  const list = demoMode ? GUEST_TOPICS : BASE_TOPICS;
  return list.find((t) => t.name === name) || GUEST_TOPICS.find((t) => t.name === name) || null;
};
export const getTopicById = (id, demoMode = isGuestModeActive()) => {
  const list = demoMode ? GUEST_TOPICS : BASE_TOPICS;
  return list.find((t) => t.id === id) || GUEST_TOPICS.find((t) => t.id === id) || null;
};

// ---- 帖子数据（基线登录态 vs 游客体验模式） ----
...
export const posts = new Proxy([], {
  get(target, prop, receiver) {
    const current = isGuestModeActive() ? GUEST_POSTS : BASE_POSTS;
    if (prop === Symbol.iterator) return current[Symbol.iterator].bind(current);
    const val = Reflect.get(current, prop, receiver);
    return typeof val === 'function' ? val.bind(current) : current[prop];
  },
...
export const getContentsByTopicName = (topicName, demoMode = isGuestModeActive()) => {
  const topic = getTopicByName(topicName, demoMode);
  if (!topic) return [];
  const list = demoMode ? GUEST_POSTS : BASE_POSTS;
  return list.filter((p) => p.topicId === topic.id && p.auditStatus === 'approved');
};

export const getMyVisiblePosts = (demoMode = isGuestModeActive()) => {
  const list = demoMode ? GUEST_POSTS : BASE_POSTS;
  return list.filter((p) => p.authorId === CURRENT_USER_ID);
};

export const deletePost = (postId) => {
  const baseIdx = BASE_POSTS.findIndex((p) => p.id === postId);
  if (baseIdx !== -1) BASE_POSTS.splice(baseIdx, 1);
  const guestIdx = GUEST_POSTS.findIndex((p) => p.id === postId);
  if (guestIdx !== -1) GUEST_POSTS.splice(guestIdx, 1);
  _comments = _comments.filter((comment) => comment.postId !== postId);
  return baseIdx !== -1 || guestIdx !== -1;
};

export const getFollowedUserPosts = (demoMode = isGuestModeActive()) => {
  const list = demoMode ? GUEST_POSTS : BASE_POSTS;
  return list.filter(
...
// 审核通过的公开帖子
export const getApprovedPosts = (demoMode = isGuestModeActive()) => {
  const list = demoMode ? GUEST_POSTS : BASE_POSTS;
  return list.filter((p) => p.auditStatus === 'approved');
};
