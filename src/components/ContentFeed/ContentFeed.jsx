...
// ─── 游客模式组件（b5ded1b：无外层Tab，直出社区二级Tab） ───
const GuestContentFeed = forwardRef(({
  showPublishPage,
  setShowPublishPage,
  ...
  const [communitySubTab, setCommunitySubTab] = useState('故障求助');
...
GuestContentFeed.displayName = 'GuestContentFeed';

// ─── 登录态基线组件（db62a7e：外层5个Tab + 轮播 + 广告 + 推荐 + 详情） ───
const BaselineContentFeed = forwardRef(({
  showPublishPage,
  setShowPublishPage,
  setIsCommunityPublishEligible,
  setIsCommunityPublishViewportActive,
  ...
  const [interactionTargetStatus, setInteractionTargetStatus] = useState(null);
  const [topicDetail, setTopicDetail] = useState(null);
  const [communitySubTab, setCommunitySubTab] = useState('我的');
  const [communityTabKey, setCommunityTabKey] = useState(0);
    ...
    if (item.type === 'community' || item.type === 'community_video') {
      return (
        <div className="bg-white rounded-[11px] overflow-hidden shadow-sm cursor-pointer" onClick={handleClick}>
          ...
          <div className="p-2">
            {item.topicId && (() => {
              const t = getTopicById(item.topicId, demoMode);
              ...
              <div className="absolute inset-0 bg-gradient-to-r from-black/70 via-black/30 to-transparent flex items-center">
                ...
                {onInquiry && (
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      trackLeadIntent.serviceEntryClick('ad_inquiry', 'homepage_ad_banner');
                      onInquiry({ campaign: adItems[currentAd].title });
                    }}
                    className="absolute bottom-2 right-3 flex h-7 items-center gap-1 rounded-md bg-brand-red px-2.5 text-[11px] font-medium text-white"
                  >
                    我要询价 <span aria-hidden="true">›</span>
                  </button>
                )}
              </div>
              <div
...
BaselineContentFeed.displayName = 'BaselineContentFeed';

// ─── ContentFeed 统一入口：根据 demoMode 分支隔离 ───
const ContentFeed = forwardRef((props, ref) => {
  if (props.demoMode) {
    return <GuestContentFeed ref={ref} {...props} />;
  }
  return <BaselineContentFeed ref={ref} {...props} />;
});
