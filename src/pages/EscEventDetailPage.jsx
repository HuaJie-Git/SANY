...
import { ESC_SIGNAL_FIELDS } from '../data/escEvent';
...
function SignalValue({ field }) {
  const value = field.value ?? '-';
  return <strong>{value}{field.unit && <small>{field.unit}</small>}</strong>;
}

function SignalSnapshot() {
  return <section className="esc-signal-panel" aria-labelledby="esc-signal-title">
    <header><div><h2 id="esc-signal-title">ESC属性快照</h2><p>属性名称已在 APP 与 PC 统一，数据值、单位和顺序保持不变。</p></div><span>共 {ESC_SIGNAL_FIELDS.length} 项</span></header>
    <div className="esc-signal-grid">
      {ESC_SIGNAL_FIELDS.map((field) => <article className={field.children ? 'is-wheel-speed' : ''} key={field.key}>
        <span dir="auto">{field.label}</span>
        {field.children ? <div className="esc-wheel-grid">{field.children.map((wheel) => <div key={wheel.key}><small dir="auto">{wheel.label}</small><SignalValue field={wheel}/></div>)}</div> : <SignalValue field={field}/>}
      </article>)}
    </div>
  </section>;
}
...
export default function EscEventDetailPage({ eventId, onBack, onSelectEvent }) {
  ...
  return <div className="esc-detail-page">
    ...
    <section className="esc-detail-device-card">
      <div className="esc-detail-device-main"><div className="esc-detail-truck" aria-hidden="true"><span>▰</span><i/><b/></div><dl><div><dt>设备编号/自编号/车牌号</dt><dd>{event.deviceCode}/-/-</dd></div><div><dt>设备类型</dt><dd>{event.deviceType}</dd></div><div><dt>关联项目</dt><dd>{event.project}</dd></div><div><dt>设备状态</dt><dd><i className={event.online === '在线' ? 'is-online' : ''}/>{event.online}　<i className={event.running === '工作' ? 'is-online' : ''}/>{event.running}</dd></div><div><dt>ESC开启状态</dt><dd>开启</dd></div></dl></div><button className="esc-detail-report" type="button" onClick={() => setToast('已生成 ESC 事件报停申请草稿')}>● 报停</button>
    ...
    <section className="esc-detail-filters" aria-label="ESC趋势筛选"><input type="date" value={date} onChange={(item) => setDate(item.target.value)} aria-label="ESC查询日期"/><input type="time" step="1" value={time} onChange={(item) => setTime(item.target.value)} aria-label="ESC查询时间"/><div><span>ESC工作状态 <button type="button" aria-label="移除 ESC 工作状态">×</button></span><span>+13项</span></div><button type="button" className="esc-detail-export" onClick={() => setToast(`${event.deviceCode} 的 ESC 报表已导出`)}>⇧ 导出报表</button></section>

    <main className="esc-detail-content"><SignalSnapshot/><StatusChart title="ESC工作状态" tone="is-work" labels={['工作', '未工作']}/><StatusChart title="ESC故障状态" tone="is-fault" labels={['异常', '正常']}/><aside className="esc-detail-context"><b>事件编号：{event.eventNo}</b><span>{eventLabel} · ESC 发生于 {event.occurredAt}</span></aside></main>
