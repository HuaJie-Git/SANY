        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/45" onClick={() => setAuditItem(null)}>
          <div className="bg-white rounded-xl w-[600px] max-h-[85vh] flex flex-col shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              <h3 className="text-[16px] font-semibold text-gray-900">审核详情</h3>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              ...
              <div className="flex text-[13px]">
                <span className="w-[70px] text-gray-500 flex-shrink-0">关联话题</span>
                <span className="text-gray-900">{auditItem.topicName}</span>
              </div>

              {/* 标题 */}
              <div className="flex text-[13px]">
                <span className="w-[70px] text-gray-500 flex-shrink-0">标题</span>
                <span className="text-gray-900">{auditItem.content}</span>
              </div>

              {/* 正文 */}
              {auditItem.contentBody && (
                <div className="flex text-[13px]">
                  <span className="w-[70px] text-gray-500 flex-shrink-0">正文</span>
                  <span className="text-gray-900 leading-relaxed">{auditItem.contentBody}</span>
                </div>
              )}
              ...
              {/* 提交时间和状态 */}
              <div className="flex text-[13px]">
                <span className="w-[70px] text-gray-500 flex-shrink-0">提交时间</span>
                <span className="text-gray-900">{auditItem.date}</span>
              </div>
              <div className="flex items-center text-[13px]">
                <span className="w-[70px] text-gray-500 flex-shrink-0">当前状态</span>
                <span className={`text-[12px] px-1.5 py-0.5 rounded ${getStatusLabel(auditItem.auditStatus).color}`}>
                  {getStatusLabel(auditItem.auditStatus).text}
                </span>
              </div>

              {/* 不通过原因（仅不通过时填写） */}
              {auditItem.auditStatus === 'pending' && (
                <div className="mt-2">
                  <label className="block text-[13px] text-gray-700 mb-1">不通过原因（选填，APP 不显示）</label>
                  <textarea
                    value={rejectReason}
                    onChange={(e) => setRejectReason(e.target.value)}
                    rows={2}
                    className="w-full px-3 py-2 border border-gray-300 rounded text-[13px] resize-none focus:outline-none focus:border-[#1890ff]"
                    placeholder="填写后仅后台留存"
                  />
                </div>
              )}
            </div>
            ...
            {auditItem.auditStatus === 'pending' && (
              <div className="px-6 py-3 border-t border-gray-100 flex items-center justify-end gap-3">
                <button
                  className="h-9 px-6 bg-green-500 text-white rounded text-[14px] font-medium hover:bg-green-600"
                  onClick={handleApprove}
                >
                  通过
                </button>
                <button
                  className="h-9 px-6 bg-orange-500 text-white rounded text-[14px] font-medium hover:bg-orange-600"
