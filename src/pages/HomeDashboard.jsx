...
export default function HomeDashboard({ onOpenDevice, onOpenList }) {
  ...
  const moduleContent = {
    tasks: <ModuleCard title="我的任务" className="is-span-7" action={<button type="button" className="module-text-action" onClick={() => openFeature(FEATURES[1])}>更多 <Icon name="arrow" size={14}/></button>}>
    ...
    audit: <ModuleCard title="审核概览" subtitle="待处理事件与未读提醒" className="is-span-5" action={<button type="button" className="module-text-action" onClick={openAudit}>进入审核 <Icon name="arrow" size={14}/></button>}>
    ...
    maintenance: <ModuleCard title="计划内维护" className="is-span-5" action={<button type="button" className="module-text-action" onClick={() => openList('全部计划内维护资产', maintenanceAssets.map((item) => item.device.id))}>查看全部 <Icon name="arrow" size={14}/></button>}>
