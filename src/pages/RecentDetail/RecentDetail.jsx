// RecentDetail 最近查看页面：隔离已登录态（db62a7e 5分类）与游客态（b5ded1b 6台车）
import React, { useState } from 'react';

const BaselineRecentDetail = ({ onBack, onNavigate }) => {
...
const GuestRecentDetail = ({ onBack, onNavigate }) => {
  // 游客模式展示当前 3 台设备（挖掘机、汽车起重机、自装卸车）
  const recentDevices = GUEST_DEMO_DEVICES.map((device) => ({
    ...device,
    type: 'asset',
    isLogo: true,
  }));
...
const RecentDetail = (props) => {
  if (props.demoMode) {
    return <GuestRecentDetail {...props} />;
  }
  return <BaselineRecentDetail {...props} />;
};
