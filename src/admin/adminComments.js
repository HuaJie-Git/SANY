...
export const COMMENT_REJECTION_REASON_MAX_LENGTH = 500;
...
export const normalizeRejectionReason = (value = '') => (
  splitVisibleCharacters(value.trim()).slice(0, COMMENT_REJECTION_REASON_MAX_LENGTH).join('')
);
...
let _comments = [
  ...
  { id: 6, postId: 4, postTitle: '吊装作业全过程记录', topicName: '#工程现场', userId: 'user-011', userName: '经验达人', content: '加我好友，领取内部低价设备资源。', createdAt: '2026-07-11 16:05:00', likeCount: 3, status: '审核未通过', rejectionReason: '广告/垃圾信息', auditTime: '2026-07-11 16:42:02', auditor: '管理员', visibility: '仅作者可见' },
...
export const rejectCommentByAdmin = (id, operator, reason) => {
  const item = _comments.find((comment) => comment.id === id);
  const normalizedReason = normalizeRejectionReason(reason);
  if (!item || item.status !== '待审核' || !normalizedReason) return false;
  item.status = '审核未通过';
  item.rejectionReason = normalizedReason;
  item.auditTime = new Date().toISOString().replace('T', ' ').slice(0, 19);
  item.auditor = operator;
  item.visibility = '仅作者可见';
  return true;
};
